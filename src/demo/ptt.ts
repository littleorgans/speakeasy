...
import { saveCorpusPair, type CorpusSidecarMeta } from "../corpus/store.ts";
...
async function main(): Promise<void> {
  ...
  // Product default: wrap with the rewrite decorator (domain rules on). The
  // engine stays pure; --no-rewrite gets raw output. --numbers adds the number
  // stage (default off). prepare()/label come from the concrete engine.
  const numbers: NumbersMode = options.numbers ?? "off";
  const voice: VoiceToText = options.noRewrite
    ? engine
    : withRewrite(engine, { rules: DEFAULT_RULES, numbers });
  const engineLabel = options.noRewrite
    ? engine.label
    : `${engine.label} +rewrite(rules=${DEFAULT_RULES.length},numbers=${numbers})`;
  const session = await voice.open({
    sampleRate: CAPTURE_SAMPLE_RATE,
    endpoint: { mode: "manual" },
  });

  const demo = new PttDemo(session, wav, options, engineLabel);
  await demo.run();
}
