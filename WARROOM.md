...
| Concept | This environment (speak-easy) |
|---------|------------------|
| Panes and message bus | tmux panes + helioy-bus (file-based bus): `register_agent`, `send_message`, `get_messages`, `nudge_message`, `whoami` |
| Orchestration entrypoint | the `/warroom` skill (`helioy-bus:warroom`), driving `warroom_spawn` / `warroom_spawn_repos` / `warroom_status` / `warroom_kill` |
| Model families for MoE | Claude runtime tiers: Fable 5, Opus 4.8, Sonnet 5, Haiku 4.5. Add a non-Claude runtime (e.g. a Codex pane) for true cross-family review; no presets defined yet (`warroom_presets` is empty) |
| Per-task gates | `pnpm typecheck` (`pnpm -r typecheck`) and `pnpm test` (`pnpm -r test`, `node --test` per package). No build or lint step yet |
| CI signal | none configured yet — verify locally via the gates, then `git push` to `origin/main` (`littleorgans/speakeasy`). Add a GitHub Actions workflow to earn a real CI status line |
| Cross-session memory | context-matters (cm) via `cx_*`, scope `global/project:helioy/repo:speak-easy`; reuse maps, plans, and decisions persist there |
